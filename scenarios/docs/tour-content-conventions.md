...
- [ ] **Beat 2 — `Mission status check — …`** is **one step** in the typical case (single `kubectl get pods …`). A second step only if the friction needs multi-resource observation to be visible. **Do not** put `describe pod` / `get events` / node-side commands here — they belong in Beat 4. The explanation opens with a **short lore intro naming the application's purpose** (mission role + who consumes it), then states the **tutorial objective** (the healthy state to look for, e.g. `Running` and `Ready`). **Do not** pre-announce the failure: the agent in Beat 3 is the canonical reveal.
